  ...
  <BikeFilterConsumer>
    {({
      ...
      const filterList = [
        {
          ...
          name: 'Kategorie',
          type: 'select',
          options: bikeCategories.map(({ name: label, id: value }) => ({
            label,
            value,
          })),
          isMulti: true,
        },
        {
          ...
          name: 'Rahmenform',
          type: 'select',
          options: bikeFrameShapes.map(({ name: label, id: value }) => ({
            label,
            value,
          })),
          isMulti: true,
      ...
      const enhancedFilterList = filterList.map(item => ({
        ...item,
        value: values[item.id].map(i => {
          const [option] = item.options.filter(({ value }) => value === i)
          return option
        }),
      }))
